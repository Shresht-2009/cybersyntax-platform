                ...
                >
                  <div className="px-5 pb-5 space-y-3">
                    ...
                    {course.students.map((student, si) => (
                      <Link key={student.id} href={`/mentor/students/${student.id}`} className="block">
                        <motion.div
                          initial={{ opacity: 0, x: -10 }}
                          animate={{ opacity: 1, x: 0 }}
                          transition={{ delay: si * 0.05 }}
                          className="flex items-center gap-4 p-3 rounded-xl cursor-pointer hover:bg-white/[0.02] transition-colors"
                          style={{ background: 'rgba(255,255,255,0.015)' }}
                        >
                          <div
                            className="w-10 h-10 rounded-full flex items-center justify-center text-sm font-semibold flex-shrink-0"
                            style={{ background: 'linear-gradient(135deg, rgba(16,185,129,0.1), rgba(245,158,11,0.1))', border: '1px solid rgba(16,185,129,0.08)' }}
                          >
